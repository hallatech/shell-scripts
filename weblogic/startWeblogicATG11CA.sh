...
DEBUG=true
USE_TAKIPI=false
LAYERS="bcc:bcc-local"
DEFAULT_LOCAL_LAYERS="bcc:bcc-local"
SWITCHING_LAYERS="bcc-switching"
LAYERS=$DEFAULT_LOCAL_LAYERS
SERVER_NAME=atg_publishing_lockerserver

function usage {
  echo "usage: startWeblogicATG11CA.sh [[ -d domain ] [-b | -f] [-x memXmx] [-n] [-r] [-s] [-l | --layers][--no-debug] [-t | --takipi] [-h]]"
  echo "  Parameters:"
...
if [ ! -d $DOMAIN_HOME ]; then
  ...
  echo "Usage: startWeblogicCA.sh [domain-name] [bg|fg] [memory(e.g.1024m)]"
  exit -1
fi
...
# ATG Server Name
if [ "x$ATG_CA_SERVER_NAME" = "x" ]; then
  ATG_CA_SERVER_NAME="AdminServer"
fi
...
JAVA_OPTIONS="${JAVA_OPTIONS} -Dweblogic.Name=${ATG_CA_SERVER_NAME} -Datg.dynamo.server.name=${SERVER_NAME} -Datg.instance.name=Publishing -Dweblogic.log.Log4jLoggingEnabled -Dlog4j.configuration=file:$LOG4J_CONFIG_FILE"

TITLE='ATG Live ('$ATG_CA_SERVER_NAME')'
...
# Logs directory
LOG_DIR=/tmp/logs
BG_LOG_FILE=nohup.weblogic.ca.out
...
# Remote Debug
if [ "$DEBUG" = "true" ]; then
  JAVA_OPTIONS="${JAVA_OPTIONS} -Xdebug -Xrunjdwp:transport=dt_socket,address=8788,server=y,suspend=n"
fi
...
printf "\nTo view server output run tailCA.sh"
printf "\nTo terminate run stopCA.sh\n\n"
